...
class HttpServices {
  ...
  static String _devURL = 'http://10.37.129.2:8810';
  ...
  static Future<bool> login({Map<String, String> emailPass}) async {
    try {
      http.Response res =
          await http.post('$serverURL/app/login', body: emailPass);
  ...
  static Future<NewUserCred> signup({NewUserCred userCred}) async {
    http.Response res = await http.post('$serverURL/app/sign-up',
  ...
  static Future<double> getBalance() async {
    try {
      ...
      http.Response res = await http.post('$serverURL/app/get-balance',
          headers: _defaultHeaders, body: jsonEncode(_buildCreds(creds)));
  ...
  static Future<bool> transferMoney(
      String destAccID, double amount, String pinCode) async {
    try {
      UserCred creds = await _prepareCredentials();
      http.Response res = await http.post('$serverURL/app/transfer-amount',
  ...
  static Future<List<Page>> getRows(
    ...
    try {
      UserCred creds = await _prepareCredentials();
      http.Response res = await http.post('$serverURL/app/get-rows',
          headers: _defaultHeaders,
          body: jsonEncode({
            ..._buildCreds(creds),
            'filters': {'CurrencyIso': currencyIso},
...
class Page {
  ...
  Page(
      {this.amount,
      this.balanceTransferType,
      this.balanceTransferTypeName,
      this.comment,
      this.currencyIso,
      this.id,
      this.insertDate,
      ...
      this.sourceAccountProfileImage,
      ...
      this.targetAccountProfileImage,
      this.targetAccountProfileImageSize,
  ...
  static Page fromJSON(Map<String, dynamic> json) {
    try {
      return Page(
          ...
          sourceAccountProfileImage:
              json['SourceAccountProfileImage'].toString(),
          sourceAccountProfileImageSize:
              int.parse(json['SourceAccountProfileImageSize'].toString()),
          ...
          targetAccountProfileImage:
              json['TargetAccountProfileImage'].toString(),
          targetAccountProfileImageSize:
              int.parse(json['TargetAccountProfileImageSize'].toString()),
...
class BalanceResponse {
  ...
  BalanceResponse(
      {this.currencyIso, this.current, this.expected, this.pending});
...
class UserCred {
  ...
  UserCred(this.credentialsToken, this.credentialsHeaderName, this.expDate,
